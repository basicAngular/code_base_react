        ...
        return (
            <div className={cx("file")} onClick={() => this.context.openFile({id: this.props.id, name: this.props.name })}>
                ...
                <div className={"fileName"}>
                    {this.props.name}
                    <br />
                    <small>Hinzugefügt: {this.props.added}</small>
                    <br />
                    <small>Größe: {bytesToSize(this.props.size)}</small>
                    ...
                    {(this.props.authStore.user.role === "krempler" || this.props.authStore.user.role === "kressist") && <div className='m-t-sm'><button className={"btn btn-warning btn-xs btn-block"} onClick={(event) => {event.stopPropagation(); this.context.modifyFile(this.props.id);}}>BEARBEITEN</button></div>}
                </div>
            </div>
        );
    }
